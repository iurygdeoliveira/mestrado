...
(function ($) {
    function fixTitle($ele) {
        if (
            ...
            $ele.attr("original-title", $ele.attr("title") || "").removeAttr(
                "title"
            );
        }
    }
    ...
    $.fn.tipsy = function (options) {
        ...
        function move(event) {
            ...
            if (options.follow == "x") {
                ...
                var isGravity =
                    /^[^w]w$/.test(options.gravity) ||
                    /^[^e]e$/.test(options.gravity);
                if (isGravity && arrow.position() != null) {
                    var x =
                        event.pageX -
                        ($(arrow).position().left + $(arrow).outerWidth() / 2);
                } else if (isGravity && arrow.position() != null) {
                    var x =
                        event.pageX -
                        ($(arrow).position().left + $(arrow).outerWidth() / 2);
                } else {
                ...
                var y = event.pageX - $(tipsy.$tip).outerWidth() / 2;
                $(tipsy.$tip).css({
                    left: x,
                });
                ...
                if (/^w|^e/.test(options.gravity)) {
                    $(tipsy.$tip).css(
                        "top",
                        event.pageY - $(tipsy.$tip).outerHeight() / 2
                    );
                }
            }
        }
